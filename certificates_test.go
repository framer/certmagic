...
func TestUnexportedGetCertificate(t *testing.T) {
	...
	// When cache is empty
	if _, matched, defaulted := cfg.getCertificateFromCache(context.Background(), &tls.ClientHelloInfo{ServerName: "example.com"}); matched || defaulted {
	...
	if cert, matched, defaulted := cfg.getCertificateFromCache(context.Background(), &tls.ClientHelloInfo{ServerName: "example.com"}); !matched || defaulted || cert.Names[0] != "example.com" {
	...
	if cert, matched, defaulted := cfg.getCertificateFromCache(context.Background(), &tls.ClientHelloInfo{ServerName: "sub.example.com"}); !matched || defaulted || cert.Names[0] != "*.example.com" {
	...
	if cert, matched, defaulted := cfg.getCertificateFromCache(context.Background(), &tls.ClientHelloInfo{ServerName: "nomatch"}); matched || defaulted {
